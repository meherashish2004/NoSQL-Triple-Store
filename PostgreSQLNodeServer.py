...
import requests
from PostgreSQLDBConnector import PostgreSQLQueryConnector  

# Instantiate MongoDBServer
postgreServer = PostgreSQLQueryConnector()
nodeHealth = True
...
def search():
    try:
        ...
        if nodeHealth:
            ...
            result, length, status = postgreServer.fetch_rows_related_to_subject(subject)
...
def update():
    try:
        ...
        if nodeHealth:
            ...
            # try:
            #     postgreServer.withinValidShard(subject, predicate, SHARDING, NODE_SHARDS)
            # except Exception as e:
            #     print('Here5',e)
            if not postgreServer.withinValidShard(subject, predicate, SHARDING, NODE_SHARDS):
                print('does not fall within a shard\n')
                abort(503)
            
            print('here2')
            # Call the update method from MongoDBServer class
            result = postgreServer.update_or_add_subject_predicate(subject, predicate, new_object)
            print('here3')
...
#         source_id = NODE_ID
#         merge_payload = postgreServer.remoteMergeLocalUpdates() 
...
def merge():
    try:
        ...
        if nodeHealth:
            ...
            result = postgreServer.mergeSelf(modifications, SHARDING, NODE_SHARDS)
...
def broadcastLocalUpdates():
    try:
        ...
        if nodeHealth:
            ...
            # Call the merge method from MongoDBServer class
            print('here10')
            result = postgreServer.remoteMergeLocalUpdates()
            print('here13')
            # Return response as JSON
